...
var Model = resource.NewModel("viam", "encoder", "incremental")
...
func (e *Encoder) Reconfigure(
	...
	e.A = encA
	e.B = encB
	e.interrupts = []string{newConf.Pins.A, newConf.Pins.B}
	e.boardName = newConf.BoardName
	e.encAName = newConf.Pins.A
	e.encBName = newConf.Pins.B
	e.board = board
	e.f, _ = os.Create("/tmp/encoder.txt")
...
func (e *Encoder) Start(ctx context.Context) {
	...
	*/
	...
	e.board.StreamTicks(context.Background(), []board.DigitalInterrupt{e.A, e.B}, chanA, nil)
